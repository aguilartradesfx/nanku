...
function Toggle({ active, onChange, disabled }: { active: boolean; onChange: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      onClick={onChange}
      disabled={disabled}
      className={`relative flex-shrink-0 w-11 h-6 rounded-full transition-colors disabled:opacity-50 ${active ? 'bg-orange-500' : 'bg-gray-300'}`}
    >
...
export default function LiveMusicManager({
  ...
  // ── Forms ─────────────────────────────────────────────────
  const EventForm = (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4 mt-3">
      <h4 className="text-gray-900 font-semibold text-sm">{editingEvent ? 'Edit Event' : 'New Event'}</h4>
      {eventFormError && <p className="text-red-500 text-xs">{eventFormError}</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">Title *</label>
          <input
            value={eventForm.title}
            onChange={e => setEventForm(p => ({ ...p, title: e.target.value }))}
            placeholder="Friday Jazz Night"
            className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-orange-400 transition"
          />
        </div>
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">Image URL</label>
          <input
        ...
        <div className="sm:col-span-2">
          <label className="block text-gray-600 text-xs mb-1.5">Subtitle / Description</label>
          <textarea
            ...
            className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-orange-400 transition resize-none"
          />
        </div>
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">CTA Button Text</label>
          <input
        ...
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">CTA Link</label>
          <input
        ...
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">Display Order</label>
          <input
            type="number"
            min={0}
        ...
        <div className="flex items-center gap-3 pt-4">
          <span className="text-gray-600 text-xs">Active on website</span>
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button onClick={cancelEventForm} className="px-4 py-2 text-gray-500 hover:text-gray-900 text-sm transition">
          Cancel
        </button>
      </div>
    </div>
  ...
  const ArtistForm = (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4 mt-3">
      <h4 className="text-gray-900 font-semibold text-sm">{editingArtist ? 'Edit Artist' : 'New Artist'}</h4>
      {artistFormError && <p className="text-red-500 text-xs">{artistFormError}</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">Name *</label>
          <input
        ...
        <div>
          <label className="block text-gray-600 text-xs mb-1.5">Label / Genre</label>
          <input
        ...
        <div className="sm:col-span-2">
          <label className="block text-gray-600 text-xs mb-1.5">Photo URL *</label>
          <input
        ...
        <div className="sm:col-span-2">
          <label className="block text-gray-600 text-xs mb-1.5">Bio (English)</label>
          <textarea
        ...
        <div className="sm:col-span-2">
          <label className="block text-gray-600 text-xs mb-1.5">Bio (Español)</label>
          <textarea
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button onClick={cancelArtistForm} className="px-4 py-2 text-gray-500 hover:text-gray-900 text-sm transition">
          Cancel
        </button>
      </div>
    </div>
      ...
      {tab === 'schedule' && (
        <div className="rounded-xl border border-gray-200 overflow-hidden bg-white">
          <div className="bg-gray-50 border-b border-gray-200 px-5 py-3.5 flex items-center gap-2">
            ...
            <h3 className="text-gray-900 font-semibold text-sm">Live Music Schedule</h3>
            <span className="text-zinc-500 text-xs ml-1">· Activar días y asignar artistas</span>
          </div>
          <div className="divide-y divide-gray-100">
            {schedule.map(day => {
              ...
              return (
                <div key={day.id} className={`px-5 py-3.5 transition ${day.is_active ? 'bg-orange-50/60' : 'bg-white'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className={`w-9 text-xs font-bold uppercase tracking-wide ${day.is_active ? 'text-orange-500' : 'text-gray-400'}`}>
                        {day.day_of_week.slice(0, 3)}
                      </span>
                      <div>
                        <p className={`text-sm font-medium ${day.is_active ? 'text-gray-900' : 'text-gray-400'}`}>{day.day_of_week}</p>
                  ...
                  {day.is_active && (
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                      <div>
                        <label className="block text-gray-500 text-[10px] mb-1 uppercase tracking-wide">Artista</label>
                        <select
                          ...
                          className="w-full bg-white border border-gray-300 rounded-lg px-2.5 py-1.5 text-gray-900 text-xs focus:outline-none focus:border-orange-400 transition"
                        >
                      ...
                      <div>
                        <label className="block text-gray-500 text-[10px] mb-1 uppercase tracking-wide">Hora de inicio</label>
                        <input
                      ...
                      <div>
                        <label className="block text-gray-500 text-[10px] mb-1 uppercase tracking-wide">Detalle</label>
                        <input
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden bg-white">
          <div className="bg-gray-50 border-b border-gray-200 px-5 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-gray-900 font-semibold text-sm">Weekly Events</h3>
              <span className="text-gray-400 text-xs ml-1">· Tarjetas promocionales en el sitio</span>
            </div>
          ...
          <div className="p-4 space-y-3">
            ...
            {events.map(event => (
              <div key={event.id}>
                <div className={`flex items-start gap-3 p-3.5 rounded-xl border transition ${event.is_active ? 'border-gray-200 bg-white' : 'border-gray-200 bg-gray-50 opacity-60'}`}>
                  {event.image_url && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {event.subtitle && <p className="text-gray-500 text-xs mt-0.5 line-clamp-1">{event.subtitle}</p>}
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button onClick={() => openEditEvent(event)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition" title="Edit">
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden bg-white">
          <div className="bg-gray-50 border-b border-gray-200 px-5 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-gray-900 font-semibold text-sm">Artists</h3>
              <span className="text-gray-400 text-xs ml-1">· Artistas que aparecen en la página</span>
            </div>
          ...
          <div className="p-4 space-y-3">
            ...
            {artists.map(artist => (
              <div key={artist.id}>
                <div className={`flex items-start gap-3 p-3.5 rounded-xl border transition ${artist.is_active ? 'border-gray-200 bg-white' : 'border-gray-200 bg-gray-50 opacity-60'}`}>
                  {artist.photo && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-900 text-sm font-medium">{artist.name}</p>
                    <p className="text-orange-500 text-xs mt-0.5">{artist.label}</p>
                    {artist.bio && <p className="text-gray-500 text-xs mt-0.5 line-clamp-1">{artist.bio}</p>}
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <Toggle
                    ...
                    />
                    <button onClick={() => openEditArtist(artist)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition" title="Edit">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                        <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                      </svg>
